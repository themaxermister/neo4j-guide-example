...
class NBA_Example:
    ...
    def _create_and_return_team_members(tx, team):
        result = tx.run("MATCH (player:PLAYER)-[:PLAYS_FOR]->(team:TEAM {name: $team_name}) RETURN {element_id: ID(player), number: player.number, name: player.name, weight: player.weight, age: player.age, height: player.height} as player", team_name=team) #### NTF
        return result.data()
...
if __name__ == "__main__":
    ...
    nba.get_all_players_in_team("LA Lakers")
