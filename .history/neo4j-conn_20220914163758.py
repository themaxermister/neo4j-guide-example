...
class NBA_Example:
    ...
    def _create_and_return_team_members(tx, team):
        result = tx.run("MATCH (player:PLAYER)-[:PLAYS_FOR]->(team:TEAM {name: $team_name}) RETURN {'id': ID(player), 'number': player.number, 'name': 'Anthony Davis', 'weight': 115, 'age': 28, 'height': 2.08} as player", team_name=team) #### NTF
        return result.data()
