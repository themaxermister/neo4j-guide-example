...
class NBA_Example:
    ...
    def check_player_exist(self, name):
        with self.driver.session() as session:
            results = session.execute_write(self._add_player_to_team, name, team)
            print(results)

    @staticmethod
    def _return_player_exist(tx, name):
        result = tx.run("MATCH (p:Player {name:$name}) WITH COUNT(p) > 0 as node_count RETURN p", name=name)
        return result
...
if __name__ == "__main__":
    ...
    nba.check_player_exist("Stephen Curry")
