...
class NBA_Example:
    ...
    def create_new_player(self, name, number, weight, height, age, team=None):
        with self.driver.session() as session:
            ...
            if team != None:
                team = session.execute_read(self._return_team, team)
                relation = session.execute_write(self._add_player_to_team, player.element_id, team.element_id)
                print(relation)
    ...
    def _create_and_return_player(tx, name, number, weight, height, age):
        result = tx.run("MERGE (player:PLAYER) "
    ...
    def _add_player_to_team(tx, player_id, team_id):
        print(player_id, team_id)
        '''
        result = tx.run("MATCH (player:PLAYER) WHERE ID(player) = $player_id "
                        "MATCH (team:TEAM}) WHERE ID(team) = $team_id "
                        "MERGE (player)-[:PLAYS_FOR]->(team) "
                        "RETURN player, team", player_id=player_id, team_id=team_id)
        '''
