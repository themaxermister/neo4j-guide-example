...
class NBA_Example:
    ...
    def create_new_player(self, name, number, weight, height, age, team=None):
        with self.driver.session() as session:
            ...
            if team != None:
                relation = session.execute_write(self._add_player_to_team, player, team)
                print(relation)
    ...
    def _add_player_to_team(tx, player, team):
        result = tx.run("MATCH (player:PLAYER) WHERE ID(player) = $player_id "
                        ...
                        "RETURN player, team", player_id=player.id, team=team.id)
        return result.single()[0]
...
if __name__ == "__main__":
    ...
    nba.create_new_player("Stephen Curry", 30, 190, 185, 32)
    nba.close()